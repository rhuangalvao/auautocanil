...
OnvifManager.prototype.initWebSocketConnection = function() {
	...
	this.ws.onclose = function(event) {
		...
		this.showMessageModal('Erro', 'A conexão WebSocket foi fechada. Verifique se o server.js está em execução. ');
	}.bind(this);
	this.ws.onerror = function(error) {
		this.showMessageModal('Erro', 'Falha ao estabelecer uma conexão WebSocket. Verifique se o server.js está em execução. ');
	}.bind(this);
...
OnvifManager.prototype.startDiscoveryCallback = function(data) {
	...
	if(n === 0) {
		this.showMessageModal('Erro', 'Nenhum dispositivo foi encontrado. Recarregue esta página para descobrir dispositivos ONVIF novamente. ')
	} else {
		this.disabledLoginForm(false);
	}
};
...
OnvifManager.prototype.connectCallback = function(data) {
	...
	if(data.result) {
		...
		this.el['btn_con'].text('Connect');
		this.el['frm_con'].show();
		this.showMessageModal('Erro', 'Falha ao conectar ao dispositivo.' + data.error.toString());
		this.device_connected = false;
	}
};
